...
def test_create_agent_seeds_version_one(client: TestClient, auth_headers: dict[str, str]) -> None:
    agent = _create_agent(client, auth_headers)
...
def test_get_agent_versions_not_found(client: TestClient, auth_headers: dict[str, str]) -> None:
    response = client.get("/api/v1/agents/nonexistent/versions", headers=auth_headers)
    assert response.status_code == 404


def test_update_agent_name_only_does_not_create_a_version(
    client: TestClient, auth_headers: dict[str, str]
) -> None:
    agent = _create_agent(client, auth_headers)
...
def test_update_agent_instructions_appends_a_version(
    client: TestClient, auth_headers: dict[str, str]
) -> None:
    agent = _create_agent(client, auth_headers)
    ...
    updated = client.patch(
        f"/api/v1/agents/{agent['id']}",
        json={"instructions": "You are a senior DBA. Prefer PostgreSQL idioms."},
        headers=auth_headers,
    )
    assert updated.status_code == 200
    ...
    assert versions[0]["instructions"] == "You are a senior DBA. Prefer PostgreSQL idioms."
    assert versions[1]["instructions"] == "You are a DBA."


def test_update_agent_model_only_appends_a_version(
    client: TestClient, auth_headers: dict[str, str]
) -> None:
    agent = _create_agent(client, auth_headers)
    ...
    assert versions[0]["model"] == "openai:gpt-4o-mini"
    assert versions[0]["instructions"] == "You are a DBA."  # unchanged field carried through


def test_rollback_agent_version_restores_instructions_and_model(
    client: TestClient, auth_headers: dict[str, str]
) -> None:
    agent = _create_agent(client, auth_headers)
    ...
    client.patch(
        f"/api/v1/agents/{agent['id']}",
        json={"instructions": "Draft instructions, not final.", "model": "openai:gpt-4o-mini"},
        headers=auth_headers,
    )
    ...
    # The rollback itself is recorded as a new version, so the version log
    # stays a complete audit trail of every live value the agent has had.
    versions = client.get(f"/api/v1/agents/{agent['id']}/versions", headers=auth_headers).json()
...
def test_rollback_agent_version_agent_not_found(
    client: TestClient, auth_headers: dict[str, str]
) -> None:
