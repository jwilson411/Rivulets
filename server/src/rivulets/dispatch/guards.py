...
timeout-based pause for agent-to-agent exchanges within a rivulet.

State lives in `rivulet_guard_state` (data-model.md) — one row per rivulet,
...
from rivulets.db.models import Message, RivuletGuardState, WorkspaceSetting
...
async def get_or_create_guard_state(db: AsyncSession, rivulet_id: str) -> RivuletGuardState:
    state = await db.get(RivuletGuardState, rivulet_id)
    if state is None:
        state = RivuletGuardState(rivulet_id=rivulet_id)
        db.add(state)
        await db.flush()
    return state


def reset_guard_state(state: RivuletGuardState) -> None:
    """FR-7.5: any human message resets all counters and resumes a paused
    rivulet, whatever tripped it."""
...
async def record_agent_message(
    ...
    *,
    ...
    stop dispatching further in this rivulet once it gets one back.
    ...
    if state.agent_exchange_count >= turn_limit:
        return _pause(state, "turn_limit", TURN_LIMIT_MESSAGE, rivulet_id)
...
def _pause(state: RivuletGuardState, reason: str, message: str, rivulet_id: str) -> Message:
    state.paused = True
