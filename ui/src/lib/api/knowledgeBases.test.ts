...
describe('knowledgeBases', () => {
	...
	it('get() GETs /knowledge-bases/:id', async () => {
		const fetchMock = mockFetch({ id: 'kb1', document_count: 0 });

		const result = await knowledgeBases.get('kb1');

		const [url] = fetchMock.mock.calls[0] as [string];
		expect(url).toBe('/api/v1/knowledge-bases/kb1');
		expect(result).toEqual({ id: 'kb1', document_count: 0 });
	});

	it('create() POSTs the definition to /knowledge-bases', async () => {
		const fetchMock = mockFetch({ id: 'kb1' });

		await knowledgeBases.create({ name: 'Docs', scope_type: 'team', team_id: 't1' });

		const [url, init] = fetchMock.mock.calls[0] as [string, RequestInit];
		expect(url).toBe('/api/v1/knowledge-bases');
		expect(init.method).toBe('POST');
		expect(init.body).toBe(JSON.stringify({ name: 'Docs', scope_type: 'team', team_id: 't1' }));
	});
	...
	it('ingestDocument() POSTs { file_id } to /knowledge-bases/:id/documents', async () => {
		const fetchMock = mockFetch({ id: 'doc1', status: 'pending' });

		await knowledgeBases.ingestDocument('kb1', 'file-1');
