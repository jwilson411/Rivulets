<script lang="ts">
	...
	// OwnerGrant-only server-side (api/deps.py) -- firing them from an
	// invite-grant session could only ever collect a 403 apiece, which is
	// exactly the "Failed to load …" error stack this page used to show.
	// Budgets are the invite-safe subset (#232 deliberately left GET
	// /budgets any-grant so caps stay visible), so only those load here
	// for a non-owner; the owner-only sections are hidden below.
	if (auth.grant === 'owner') {
		refresh();
		refreshProviders();
		refreshHitRate();
...
<div class="mx-auto flex max-w-2xl flex-col gap-8 px-6 py-8">
	...
	{#if auth.grant !== 'owner'}
		<!-- #351: the invite-safe subset is just the Budgets section below --
		     every other section on this page reads owner-only endpoints. -->
		<p class="text-sm text-neutral-600 dark:text-neutral-400">
			Only budget status is visible to your session — the rest of Settings is owner-only.
		</p>
	{/if}
	...
	{#if auth.grant === 'owner'}
		...
		>
			...
			{:else}
				...
				{:else if updateStatus.update_available}
					...
					{:else}
						<p class="text-xs text-neutral-600 dark:text-neutral-400">
							Auto-update isn't available in this environment. Docker users should pull the new
							image; a source/dev install should pull the new tag.
						</p>
					{/if}
				{:else}
		...
		>
			...
			<p class="text-xs text-neutral-600 dark:text-neutral-400">
				...
				attachments in <code class="font-mono">files/</code> aren't included — cover those with your own
				filesystem backups.
			</p>
		...
		{:else}
			<form onsubmit={handleSave} class="flex flex-col gap-8">
						...
						{:else}
							...
							{#if hitRate.fallback_warning}
								...
								>
									...
									costing more than expected. Consider tightening agent descriptions or adding
									manual routing rules (FR-3.3/FR-3.4) to route more messages deterministically.
								</p>
							{/if}
						{/if}
					</div>
				...
				>
					...
					<p class="text-xs text-neutral-600 dark:text-neutral-400">
						Eager sync pushes file attachments to peers immediately; lazy sync waits until a peer
						asks for them (OQ-5). Set independently per network.
					</p>
					<label
						class="flex items-center gap-2 text-sm text-ink dark:text-ink-dark"
						title="LAN = peers reached at a local-network address (mDNS discovery, RFC1918/link-local ranges). Peers reached over Tailscale or another VPN/overlay count as WAN, not LAN. On: file attachments are pushed to LAN peers as soon as you upload/attach them. Off: LAN peers only fetch a file's bytes the moment they actually need it."
					>
					...
					<label
						class="flex items-center gap-2 text-sm text-ink dark:text-ink-dark"
						title="WAN = peers connected over the internet or an overlay network (Tailscale/WireGuard) rather than your local network. On: file attachments are pushed to WAN peers immediately, which can use significant bandwidth on a slow or metered connection. Off (default): WAN peers only fetch a file's bytes when they actually need it."
					>
