...
| **Catch-up (offline reconcile)** | Full-state snapshot push over a dedicated libp2p stream | Gossipsub only reaches peers in the mesh at publish time, so on every new peer connection (and periodically as anti-entropy) each node streams its entire synced state — current entities plus tombstones reconstructed from the durable vector-clock table — to the peer. Every snapshot envelope goes through the same vector-clock apply path as a live gossip message, so replays are idempotent (already-seen versions no-op, stale ones are ignored, concurrent ones surface as conflicts). This is what makes "nodes work fully offline and reconcile when reconnected" true: a second machine joining with the same recovery phrase receives everything published before it existed, and a peer offline during a delete receives the tombstone instead of keeping the row forever. An envelope too large for one snapshot frame is chunked across consecutive frames rather than skipped, and envelopes for an already-settled conflict re-carry the resolution stamp so a late joiner adopts the settled state instead of re-opening the conflict. |
| **File transfer** | Full-content request/response over a dedicated libp2p stream | Content-hash comparison means a peer that already has a given hash never re-requests it; no byte-level delta transfer. Fetch timing is configurable — eager on LAN by default (`sync.eager_files_lan`), lazy (on-demand) elsewhere (`sync.eager_files_wan`). "LAN" means the peer's address is in RFC1918, loopback, link-local, or IPv6 ULA space — the ranges mDNS discovery can land in. Overlay/VPN addresses, including Tailscale's CGNAT range `100.64.0.0/10`, count as **WAN**: Tailscale/WireGuard is the cross-network path, so eager transfer over it is opt-in via `sync.eager_files_wan`. Files are capped at 100 MB (FR-10.1, inclusive: exactly 104,857,600 bytes is accepted, one more byte is rejected), enforced identically at the HTTP upload path (`api/files.py`) and against the peer-declared length here (`sync/file_transfer.py`) — audited #522; the UI carries no copy of the limit (no pre-check or size hint), so there is no client-side constant to drift. |
...
| **Sandboxing** | firejail (Linux), sandbox-exec (macOS), AppContainer + job object (Windows) | Platform-native sandboxing for the Code Execution tool. If a platform's sandbox backing is missing (e.g. firejail not installed), the tool reports itself unavailable rather than running unsandboxed. |
