...
| **Integrations** | First-party Google tools + owner OAuth (#458, #463) | Tokens live in the credential store (same as provider keys), never in SQLite or a sync payload. Connecting an account is owner-only; read tools need assignment plus `integrations:google`, send/draft/create need `integrations:google:write` on that agent. |
