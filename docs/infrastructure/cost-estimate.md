# Rivulets — Cost Estimate & Infrastructure as Code

---

## Project Infrastructure Costs

Rivulets has **zero cloud hosting costs** — it's a desktop application. The costs below are for the open-source project infrastructure: source control, CI/CD, distribution, and web presence.
...
| **Domain** (`rivulets.dev`) | Namecheap / Cloudflare | ~$1.25 | ~$15/year. Standard .dev TLD pricing. |
| **Website hosting** | GitHub Pages / Cloudflare Pages | $0 | Static site. Free tier is more than sufficient. |
| **Install script hosting** | Same as website | $0 | `get.rivulets.dev` → static file on Pages. |
| **Total** | | **$0 – $5.25/month** | |
...
Users pay their own LLM provider costs directly. Rivulets does not proxy or resell API access.
...
Since Rivulets has no cloud infrastructure, traditional IaC (Terraform, Pulumi) is not applicable. The "infrastructure" that IS managed as code:
...
- Repo: `rivulets/website/`
- Host: Cloudflare Pages (free) with automatic deploy on push to `main`.
- Custom domain: `rivulets.dev`.
- Subdomain `get.rivulets.dev` serves the install script.

### DNS Configuration

```
rivulets.dev        → Cloudflare Pages (website)
get.rivulets.dev    → Cloudflare Pages (install script redirect)
docs.rivulets.dev   → Cloudflare Pages (docs site — could be same as website)
```
...
This produces a signed attestation verifying that the binary was built from a specific commit on the official repo — users can verify they're running the real Rivulets, not a tampered binary.
