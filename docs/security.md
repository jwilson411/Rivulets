...
Layer 2b: Invite-Based Access
  └─ A second human can join without ever seeing the workspace mnemonic.
     ...
     invite-redeemed session can never reach. Starter agents (Assistant,
     Coder, Researcher, Writer) ship without admin or sensitive
     capability scopes, so an invited guest chatting with always-on
     Assistant cannot retarget the project folder, send mail, mint
     invites, or run code unless the owner later granted those tools on
     that agent. Invites are never P2P mesh
...
Layer 5: Credential Isolation
  └─ LLM provider keys live in the OS keychain, never in the database.
     ...
     this so the tradeoff (below) is informed, not silent. Integration
...
- **Auth:** sessions use a bearer JWT in the `Authorization` header, held in browser memory (not `localStorage`/`sessionStorage`) — never an ambient cookie, so there's no CSRF-style attack surface to defend against. Two deliberate exceptions persist *re-entry credentials* (never the JWT) so a refresh or typed URL does not dump the user on Unlock:
  - **Invite resume (#350):** an invite-redeemed browser persists an *invite resume token* in `localStorage` — a per-redemption secret exchanged at `POST /invites/resume` for a fresh invite-grant session. Without it, a refresh or sign-out would permanently lock out an invited human, who has no mnemonic and can't re-redeem a spent single-use invite. The token is scoped (it can only ever mint `grant="invite"` sessions, never owner access), bcrypt-hashed at rest server-side, expires after 30 days idle, and dies when the owner revokes the invite it came from — and the invited browser already persists a comparable secret regardless, since the original invite URL sits in its history.
  - **Owner stay-signed-in (#407):** an owner who checks "Stay signed in on this machine" persists the recovery phrase (and optional passphrase) in `localStorage` so a refresh, new tab, or typed URL can `POST /auth/login` again and re-claim the last identity. Off by default; explicit sign-out drops it. This is a real XSS/local-access tradeoff — the checkbox copy discloses that the phrase is stored in the browser, and leaving it unchecked still warns that refresh signs you out.
...
| XSS in the web UI reads the session token | JWT held in memory, not `localStorage`; CSP headers; Svelte's compile-time output escaping | A DOM-based XSS via a vulnerable dependency is still possible — keep dependencies current. An owner who opted into stay-signed-in (#407) also has the recovery phrase in `localStorage`, which that same XSS could read — the checkbox discloses this, and sign-out removes it. |
...
| An invited human's device is later compromised | Invite-grant sessions are owner-gated out of provider credentials, backups, sync control, further invites, and integration connect/disconnect (#458); they never gain P2P mesh membership, so there's nothing to exfiltrate beyond that session's own JWT and the persisted invite resume token (#350). Starter agents do not receive admin/sensitive scopes (#462), so guest chat cannot drive the owner's disk, mailbox, or invite keys through Assistant unless the owner granted those tools. | An attacker with that JWT can act as that human within the un-gated surface (channels, messages, agents) until the token expires, and the resume token in `localStorage` lets them mint further invite-grant sessions until it idles out (30 days) — but revoking the invite kills the resume token on its next use, so the owner *does* have a remote kill switch for the persistent credential, unlike for an individual in-flight JWT. An owner who later grants `settings:manage`, `integrations:google`, or `sensitive_tools:manage` to an always-on agent re-opens that tool path for anyone who can talk to it. |
| A Google OAuth refresh token is stolen from the credential store | Same store as provider keys (OS keychain, or mnemonic-derived fallback). Disconnect revokes the token at Google and deletes the secret so tools fail closed. | Same residual as provider keys: a root-level attacker on the machine can read the keychain. |
