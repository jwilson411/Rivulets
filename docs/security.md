...
| A node bound to `0.0.0.0` (Docker) is reachable before the owner's first login and an attacker races them to `POST /auth/login` with their own mnemonic (#247) | `RIVULETS_BOOTSTRAP_TOKEN`: creating the workspace row while bound to `0.0.0.0` requires the request to carry a matching operator-set token; unset, first login over `0.0.0.0` refuses to bootstrap at all | An operator who exposes the node beyond loopback without setting the token can't complete first login at all (fails closed) until they set it — a deliberate inconvenience over the alternative of leaving the race open. |
